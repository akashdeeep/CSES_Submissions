...
bool check(P a, P b, P c, P d) {
    ...
    int cp3 = cross(c, d, a);
    int cp4 = cross(c, d, b);
    if (cp1 * cp2 < 0 and cp3 * cp4 < 0) return 1;
    if (cp3 == 0 and mid(c, d, a) and cp4 < 0) return 1;
    if (cp4 == 0 and mid(c, d, b) and cp4 < 0) return 1;
    return 0;
}
...
void solve() {
    ...
    int ans = 0;
    for (int i = 0; i < n; i++) {
        int x1 = arr[i].first, y1 = arr[i].second, x2 = arr[(i + 1) % n].first, y2 = arr[(i + 1) % n].second;
        ans += x1 * y2;
        ans -= x2 * y1;
    }
    ans = abs(ans);
    int b = 0;
    for (int i = 0; i < n; i++) {
        int gcd = __gcd(arr[(i + 1) % n].first - arr[i].first, arr[(i + 1) % n].second - arr[i].second);
        b += abs(gcd);
    }
    int a = ans + 2 - b;
    a /= 2;
