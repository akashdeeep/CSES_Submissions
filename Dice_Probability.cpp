...
double foo(int n, int sum) {
    ...
    if (sum <= 0) return 0;
    if (sum > n * 6 or sum < n) return 0;
    if (mp.find({n, sum}) != mp.end()) return mp[ {n, sum}];
    double temp = foo(n - 1, sum - 1) / 6 + foo(n - 1, sum - 2) + foo(n - 1, sum - 3) + foo(n - 1, sum - 4) + foo(n - 1, sum - 5) + foo(n - 1, sum - 6);
    temp /= 6.00;
    // cout << n << " " << sum << " " << temp << endl;
    return mp[ {n, sum}] = temp;
}
...
void solve() {
    ...
    for (int j = 1; j <= n; j++) {
        for (int sum = 1; sum < 601; sum++) {
            for (int k = 1; k < 7; k++) {
                if (sum - k >= 0) {
                    dp[sum][j] += dp[sum - k][j - 1] * (1.00 / 6);
                }
            }
        }
    }
