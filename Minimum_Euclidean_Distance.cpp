...
void solve() {
    ...
    int d = 7999999984000000008;
    sort(arr.begin(), arr.end());
    set<pii> s = {{arr[0].S, arr[0].F}};
    int j = 0;
    for (int i = 1; i < n; i++) {
        auto it = s.begin();
        int dd = ceil(sqrt(d));
        while (j < i and arr[j].F < arr[i].F - dd) {
            s.erase({arr[j].S, arr[j].F});
            j++;
        }
        auto l = s.lower_bound({arr[i].S - dd, 0});
        auto r = s.upper_bound({arr[i].S + dd, 0});
        for (auto it = l; it != r; it++) {
            d = min(d, norm({it->S, it->F}, arr[i]));
        }
