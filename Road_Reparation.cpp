...
int par(vector<pair<int, int>> &p, int node) {
    int ans = node;
    while (p[ans].first != ans) {
        ans = p[ans].first;
    }
    return ans;
}
...
void solve() {
    ...
    while (!pq.empty()) {
        ...
        int cost = temp[0], node1 = temp[1], node2 = temp[2];
        if (vis[node1] and vis[node2]) continue;
        ans += cost;
        if (!vis[node1]) {
            for (int new_ind : adj[node1]) {
                pq.push(edges[new_ind]);
            }
            // cout << node1 << endl;
            vis[node1] = 1;
        }
        if (!vis[node2]) {
            for (int new_ind : adj[node2]) {
                pq.push(edges[new_ind]);
            }
            // cout << node2 << endl;
            vis[node2] = 1;
