...
void solve() {
    ...
    dfs(0, -1);
    for (int i = 0; i < n; i++) {
        temp_arr[i] = arr[temp_ind[i]];
    }
    for (int i = 0; i < n; i++) {
        rev_ind[temp_ind[i]] = i;
    }
    // for (int i = 0; i < n; i++) {
    //     cout << temp_ind[i] << " ";
    // }
    ...
    while (q--) {
        ...
        if (ver == 1) {
            int k, x; cin >> k >> x;
            update(rev_ind[k - 1], x);
        }
        else {
            int s; cin >> s;
            // cout << rev_ind[s - 1] << " " << s << " " << tree_size[s - 1] << endl;
            cout << find(rev_ind[s - 1], rev_ind[s - 1] + tree_size[s - 1] - 1) << endl;
        }
    }
}
