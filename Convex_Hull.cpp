...
bool check1(vii &up, pii coor) {
    pii temp1 = up.back(), temp2 = up[up.size() - 2];
    pii a = {temp2.first - temp1.first, temp2.second - temp1.second};
    pii b = {coor.first - temp2.first, coor.second - temp2.second};
    int m = a.F * b.S - a.S * b.F;
    if (m < 0) return true;
    return false;
}
bool check2(vii &up, pii coor) {
    pii temp1 = up.back(), temp2 = up[up.size() - 2];
    pii a = {temp2.first - temp1.first, temp2.second - temp1.second};
    pii b = {coor.first - temp2.first, coor.second - temp2.second};
    int m = a.F * b.S - a.S * b.F;
...
void solve() {
    ...
    vii up = {arr[0], arr[1]};
    for (int i = 2; i < n; i++) {
        while (up.size() >= 2 and check1(up, arr[i])) {
            up.pop_back();
        }
        up.push_back(arr[i]);
    }
    vii down = {arr[0], arr[1]};
    for (int i = 2; i < n; i++) {
        while (down.size() >= 2 and check2(down, arr[i])) {
            down.pop_back();
        }
