...
void solve() {
    ...
    int ans2 = 1;
    for (auto &i : mp) {
        ans2 *= (power(i.first, i.second + 1, mod) + mod - 1) % mod;
        ans2 %= mod;
        ans2 *= power(i.first - 1, mod - 2, mod);
        ans2 %= mod;
    }
    int expo = 1;
    bool ver = false;
    ...
    if (ver == true) {
        for (auto i : mp) {
            N *= power(i.first, i.second, mod);
            N %= mod;
        }
        ans3 = power(N, expo, mod);
    }
    else {
        for (auto i : mp) {
            N *= power(i.first, i.second / 2, mod);
            N %= mod;
        }
        ans3 = power(N, expo, mod);
    }
