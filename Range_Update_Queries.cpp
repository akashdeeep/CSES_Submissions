...
int find_sum(int a, int b, vector<int> &seg_tree) {
    if (a < 0) return 0;
...
void add(int k, int u, vector<int> &seg_tree) {
    int n = seg_tree.size() / 2;
    k += n;
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++) {
        cin >> temp[i];
        temp2[i - 1] = temp[i] - temp[i - 1];
    }
    for (int i = 0; i < n; i++) {
        seg_tree[n + i] = temp2[i];
    }
    for (int i = n - 1; i > 0; i--) {
    ...
    while (q--) {
        ...
        if (ver == 1) {
            ...
            int k; cin >> k;
            cout << find_sum(0, k - 1, seg_tree) << endl;
        }
    }
