...
void dfs1(int node, vector<vector<int>> &adj, int par, vector<int> &children) {
    int temp = 1;
    for (auto &i : adj[node]) {
        if (i != par) {
            dfs1(i, adj, node, children);
        }
        temp += children[i];
    }
    children[node] = temp;
}
int dfs2(int node, vector<vector<int>> &adj, int par, vector<int> &children) {
    int ans = 0;
    for (auto &i : adj[node]) {
        if (i != par) {
            ans += children[i] + dfs2(i, adj, node, children);
        }
    }
    return ans;
}

void dfs3(int node, vector<vector<int>> &adj, int par, vector<int> &dis, vector<int> &children) {
    for (auto &i : adj[node]) {
        if (i != par) {
            dis[i] = dis[node] - children[i] + (adj.size() - children[i]);
        }
    }
    for (int &i : adj[node]) {
        if (i != par) {
            dfs3(i, adj, node, dis, children);
        }
    }
}
...
void solve() {
    ...
    dfs1(0, adj, -1, children);
    vector<int> dis(n, -1);
    dis[0] = dfs2(0, adj, -1, children);
    dfs3(0, adj, -1, dis, children);
    for (int i : dis) {
        cout << i << " ";
    }
}
